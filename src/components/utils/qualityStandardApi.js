...
export async function createQualityStandard({productId,description,minValue,maxValue,unit}){
    try{
        ...
        if(
            ...
            isNaN(parseMinValue) || parseMaxValue <= 0 || 
            isNaN(parseMaxValue) || parseMaxValue <= 0 || !isUnitValid.includes(unit?.toUpperCase())){
...
export async function searchQualityStandards({supplyId, productStorageMin, productStorageMax, supplyMin, supplyMax, shelfRow, shelfCol}){
    try{
        ...
        if(
            isNaN(supplyId) || supplyId == null || isNaN(parseProductStorageMin) || parseProductStorageMin <= 0 || 
            isNaN(parseProductStorageMax) || parseProductStorageMax <=0 ||
            isNaN(parseSupplyMin) || parseSupplyMin <= 0 || isNaN(parseSupplyMax) || parseSupplyMax <= 0 || 
            isNaN(parseShelfRow) || parseShelfRow <= 0 ||
...
export async function findByProduct_ShelfRowAndColBetweenNullable({rowMin, rowMax, colMin, colMax}){
    try{
        ...
        if(
            isNaN(parseRowMin) || parseRowMin <= 0 || isNaN(parseRowMax) || parseRowMax <= 0 || 
            isNaN(parseColMin) || parseColMin <= 0 || isNaN(parseColMax) || parseColMax <= 0){
