...
const url = `${import.meta.env.VITE_API_BASE_URL}/rawMaterials`;
...
export async function createRawMaterial({name, unitMeasure, supplierType, storageType, goodsType, storageId, supplyId, currentQuantity, productId, barCodes}){
    try{
        ...
        const requestBody = {name,unitMeasure,supplierType,storageType,goodsType,storageId,supplyId, currentQuantity,productId, barCodes};
        const response = await api.post(url+`/create/new-rawMaterial`,requestBody,{
            headers:getHeader()
        });
...
export async function findAllRawMaterials(){
    try{
        const response = await api.get(url+`/get-all-raw-materials`,{
            headers:getHeader()
        });
        return response.data;
    }
...
export async function findByName(name){
    try{
        ...
        const response = await api.get(url+`/find-by-material-name`,{
            params:{
                name:name
            },
            headers:getHeader()
...
export async function findBySupplierType(supplierType){
    try{
        ...
        const response = await api.get(url+`/supplierType`,{
...
export async function findByStorageType(storageType){
    try{
        ...
        const response = await api.get(url+`/storageType`,{
...
export async function findByGoodsType(goodsType){
    try{
        ...
        const response = await api.get(url+`/goodsType`,{
